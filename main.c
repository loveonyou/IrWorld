...
int main(void) {
    ...
    bool app_status = true; // value for stay a live the app
    int backup_key, nav_choose;
    int  app_create_value = builder_app(&backup_key); // setup the app 
    if (app_create_value == 1){ // check if the app setup corectly
    ...
    else if (app_create_value == 0){  // start of app 
